...
// Data Processing
const lessThanTen = acumulator
  .filter((item) => item < 10)
  .reduce((a, b) => a + b, 0);
const greaterThanOneHundred = acumulator
  .filter((item) => item > 100)
  .reduce((a, b) => a + b, 0);
const divGreater = acumulator.filter((item) => item > 100).length;
// Output
console.log(`The sum of the values less than 10 are...${lessThanTen}`);
console.log(
  `The Average of the numbers greater than 100 are...${
    greaterThanOneHundred / divGreater
  }`
);
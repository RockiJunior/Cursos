...
let fullName = 'Gabriel Villarroel'; //guardo mi nombre

let selectSpan = document.querySelector('#createdBy');
//traigo el selector que necesito 
//y lo  guardo en una variable...
selectSpan.innerHTML = selectSpan.innerHTML + ' ' + fullName;


//selecciono el selector y en el inner HTML le sumo mi nombre seteado arriba.
...
function ToDo(description) {
    ...
    this.complete = false;
    // esta funcion Clase instancia objectos ToDo...
}
...
ToDo.prototype.completeToDo = function() {
    this.complete = true; //agrego un metodo a la clase ToDo
}
...
function buildToDo(todo, index) {
    // Tu código acá:
    // new element todoshell
    let toDoShell = document.createElement('div'); //creo un elemento div.
    toDoShell.className = 'toDoShell'; // le coloco la clase toDoShell

    // new element todotext
    let toDoText = document.createElement('span'); //creo un elemento span

    toDoText.innerHTML = todo.description; // seteo el innerHTML con el valor de todo.description

    toDoText.id = index; //seteo el id de toDoText con el valor del index referenciado en el argumento

    if (todo.complete = true) { //si todo.complete = true, que le nombre de la clase sea completeText
        toDoText.className = 'completeText';
    }
    toDoShell.appendChild(toDoText); //seteo a toDoText como hijo de toDoShell
    toDoText.onclick = completeToDo;
    return toDoShell; //retorno el toDoShell
}
...
function addToDo() {
    ...
    let obj = new ToDo(getInput.value);
    toDoItems.push(obj);
    getInput.value = "";
    displayToDos();
}
